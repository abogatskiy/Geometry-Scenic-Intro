...
\begin{figure}[tp]
    ...
    \begin{tikzpicture}[>=Stealth, scale=0.9]
        % Diagram base (ellipse centered at (0,-1.5))
        \draw (0,-1.5) ellipse (2cm and 0.8cm);
        \node at (3.3,-1.5) {Diagram $F(\calJ)$};

        % Vertices inside the ellipse
        \coordinate (v1) at (-1.2,-1.5);
        \coordinate (v2) at (-0.5,-1.25);
        \coordinate (v3) at (0.1,-1.6);
        \coordinate (v4) at (0.7,-1.3);
        \coordinate (v5) at (1.2,-1.5);
        \foreach \v in {v1,v2,v3,v4,v5} { \fill (\v) circle (1.5pt); }

        % Internal arrows of the diagram
        \draw[->] (v1) -- (v2);
        \draw[->] (v2) -- (v3);
        \draw[->] (v3) -- (v4);
        \draw[->] (v4) -- (v5);
        \draw[->, bend left=20] (v1) to (v3);

        % Limit object L (upper right)
        \fill (3,2) circle (2pt);
        \node[above=2pt] at (3,2) {$L$};

        % Limit cone arrows from L (red)
        \foreach \v in {v1,v2,v3,v4,v5} { \draw[->, red, thick] (3,2) -- (\v); }
        \node[red] at (4.8,1.0) {Limit cone};

        % Cone apex A (upper left)
        \fill (-3,2) circle (2pt);
        \node[above=2pt] at (-3,2) {$A$};

        % Cone arrows from A (blue)
        \foreach \v in {v1,v2,v3,v4,v5} { \draw[->, blue] (-3,2) -- (\v); }
        \node[blue] at (-4.5,1.0) {Cone};

        % Universal arrow from A to L
        \draw[->, dashed] (-3,2) -- (3,2) node[midway, above=4pt] {$\exists!\, u$};
    \end{tikzpicture}
    \caption{Limit of a functor $F:\calJ\to \calC$ as the terminal cone over the diagram $F(\calJ)$.}
    \label{fig:limits}
\end{figure}
...
A dual result exists in cohomology. 
