...
\begin{rem}
    It is hard to miss the fact that the expression in \eqref{eq 1.4.9 RS2 structural} is exactly the same as in the Maurer-Cartan equation. The connection between them will be intuitively explained by Cartan geometry in Chapter~\ref{ch: cartan geom}, even though the above result is, strictly speaking, more general. The upshot will be that the principal bundle in Cartan geometry can be locally identified with a Lie group in a way analogous to Klein geometry, and the connection form on it can be seen as a deformation of the Maurer-Cartan form.
\end{rem}
...
\begin{rem}
    \begin{enumerate}
        ...
        \item There is another mechanism by which curvature forms may be globally pulled back to the base, namely when the principal bundle in question is \emph{natural}, i.e., a subbundle of some frame bundle, thus defining a Cartan geometry. As we will see later, the existence of the soldering isomorphism in such geometries allows one to identify certain equivariant forms on the bundle with forms on the base manifold (by identifying the space in which the form takes values, e.g., a Lie algebra, with the fiber of some tensor bundle).
    \end{enumerate}
\end{rem}
...
Now, since a linear connection on $E$ amounts to a specification of parallel sections, we can define the corresponding principal connection on the frame bundle $\Fr(E)$ by declaring a local frame (local section of $\Fr(E)$) to be parallel iff it consists of $k$ parallel sections. In particular, linear connections on the tangent bundle $\T M$ are in one-to-one correspondence with principal connections on the frame bundle $\Fr(M)$. The latter connections are briefly referred to as \emph{linear}, or \emph{affine, connections on $M$}. We will use the word \emph{affine} to indicate the focus on the tangent bundle (and other natural bundles) of the base manifold, but its meaning will become much more precise in the context of Cartan connections.
...
\begin{rem}\label{rem 2.1.17 RS2}
    \begin{enumerate}
        ...
        \item Clearly $\nabla_X$ has all the properties listed in Proposition~\ref{prop 1.5.8 RS2}. Moreover, it induces covariant derivatives on all tensor bundles $\bbT^k_l M$. A general formula is easily derived from \eqref{eq 1.4.2 RS2} by taking $\sigma$ for the tensor product representation of $k$ copies of $\sigma_n^0$ and $l$ copies of its dual.
    \end{enumerate}
\end{rem}
...
\begin{example}[\ref{ex connections on G, part 3} continued]\label{ex connections on G, part 4}
    ...
    \[\wt\gamma^-(t)=\int_0^t \delta\gamma(s)\dd s \quad \in \frakg.\]
    ...
    This formula is also true for $\sfT^+$ if $\wt\gamma^+$ is defined using right translations. Here, $\theta_G$ merely acts so that the values of the integrand belong to one vector space, $\frakg$. In local coordinates, this integral is proportional to the value of $\sfT^-$ at the identity times the area of $S$ in the limit of small $\gamma$ and $S$. This result, in fact, holds for arbitrary affine connections (although in general there is no analog of $\theta_G$ that can transport all values of $\sfT$ to one point, so the general formula can only be written in local coordinates). Therefore, torsion can be interpreted as the ``displacement of the origin'' of a tangent space as it is being \emph{rolled without slipping}\index{Rolling without slipping} along an infinitesimal loop (this is easier to imagine as $M$ rolling on $\bbR^n$ so that the point of contact follows $\gamma$). For this reason the soldering form $\theta$ is also sometimes called the \emph{displacement form}.\index{Displacement form} This interpretation will be clarified and generalized in Cartan geometry, where rolling coincides with parallel transport. We return to these connections in Example~\ref{ex connections on G, part 5}.
\end{example}
...
\begin{rem}
    This statement should not be confused with the existence of Levi-Civita connections. So far we have not imposed any compatibility constraints on the connections. In the presence of such constraints torsion-free connections may not exist at all. In the context of metric connections we will derive the existence of a unique torsion-free metric connection from the general theory of $G$-structures in \S\ref{sec: connections on G-structures}.
\end{rem}
...
\begin{defn}[Principal connection of a reductive geometry]
    ...
    \[\omega^\frakm\coloneqq \eta_\frakh\quad \in\Omega^1(P;\frakh)^\Ad.\]
...
\begin{defn}[Infinitesimal model]
    An infinitesimal model of for Cartan geometries is a pair $(\frakg,H)$, where $H$ is a Lie group with Lie algebra $\frakh$ and $\frakg$ is a Lie algebra which is also an $H$-module containing $\frakh$ such that $H$ acts by Lie algebra automorphisms extending its adjoint action on $\frakh$. Even more generally, $\frakg$ doesn't have to be a Lie algebra, but can merely be an $H$-module as above.
\end{defn}

The definition of a \emph{Cartan geometry infinitesimally modeled on $(\frakg,H)$} is exactly the same as the original definition of a Cartan geometry, except with the group $G$ forgotten. This brings the concept closer to what we saw in Cartan's equivalence method, where the Cartan connection is just an $H$-equivariant coframe on $P$, and so its values lie only in some abstract $H$-module.
